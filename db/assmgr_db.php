...
class assmgr_db_functions extends assmgr_logging {
    ...
    function get_block_course_ids($course_id=null) {
        if (!empty($course_id) && is_array($course_id)) {
           $course_condidtion = ' AND c.instanceid IN ('.implode(',',$course_id).')';
        } else {
            $course_condidtion = (!empty($course_id)) ? " AND c.instanceid = {$course_id}" : '';
        }
        ...
        $sql = "SELECT  c.instanceid
                 FROM   {block_instances} as bi,
                        {block} as b,
                        {context} as c
                 WHERE  bi.parentcontextid = c.id
                 AND	b.name = bi.blockname
    ...
    /**
     * Get the Moodle module given by id.
     *
     * @param int $module_id The id of the module
     * @return object the moodle module 
     */
                        ...
                        FROM (
                            ...
                            SELECT gg.userid,
                             ...
                             WHERE gi.itemtype = '".GRADE_ASSMGR_ITEMTYPE."'
                               AND gi.itemmodule IS NULL
                               AND gi.courseid = {$id}
                               AND gi.iteminstance IS NOT NULL
                               AND gi.outcomeid IS NULL
                               AND gi.id = gg.itemid
                               AND gg.finalgrade IS NOT NULL
                          GROUP BY gg.userid
...
/**
   ...
   /**
    ...
    function get_log_matrix_verification($verification_ids, $flextable) {
     ...
     /**
     * Returns a matrix of activities created in he given course 
     *
     * @param int 		$course_id the id of the course that the activities will be taken from 
     * @param object 	$flextable the table where the matrix will be displayed
     * @param int		$group_id  the id of the group that the user is in  
     * @return array the matrix as an array of row objects
     */   
    function get_course_activities_matrix($course_id,$group_id=null)	{
    	
    	$groupfrom 		=	(!empty($group_id)) ? ", {groupings_groups}  as grp ": " " ;
    	$gourpsql		=	(!empty($group_id)) ? " AND (cm.groupmode = 0 OR (cm.groupmode = 0 AND grp.groupingid = cm.groupingid AND grp.groupingid = {$group_id})) " : " AND cm.groupmode = 0 ";
    	
    	
    	$select		=	"SELECT 		cm.id 		AS 	cm_id,
										gi.itemname AS 	name,
										m.name		AS	modulename,
										cm.instance AS  cminstance,
										c.id 		AS  course_id ";
    
    	$from		=	"FROM 			{course_modules}	AS	cm,
										{course}			AS	c,
										{modules}			AS	m,
										{grade_items}		AS	gi
										$groupfrom ";
    	
    	$where		=	" WHERE			c.id		=	cm.course
						 	AND			cm.module	=	m.id
        ...
        return $this->dbc->get_records_sql($select.$from.$where.$sort);
   ...
   /**
    ...
    /***********************************
     * New Assmgr 2.5 functions 
     */
    
    /**
     * 
     * Returns a outcome record from the grade_items table
     * @param int $course_id	the id of the course that the outcome is in
     * @param int $modulename	the name of the module that the outcome is attached to 
     * @param int $instance_id	the instance of the module (taken from course_module table)
     * @param int $outcome_id	the id of the outcome that we are looking for
     * 
     * @return 	mixed object the grade_item record that matches the given params or false  
     */
    function get_activity_outcome($course_id,$modulename,$instance_id,$outcome_id) {
    	...
    	$sql = "SELECT 	*
                 ...
                 WHERE 	courseid		= 	:course_id
                   ...
                   AND	itemtype		=	'mod'
                   AND	iteminstance	=	:instance_id
                   AND	outcomeid		=	:outcome_id";

        return $this->dbc->get_record_sql($sql,array('course_id'=>$course_id,
        											 'module'=>"$modulename",
     ...
     * @return 	mixed object the grade_item record that matches the given params or false  
     */
    function get_overall_course_outcome($course_id,$outcome_id) {
        ...
        return $this->dbc->get_record_sql($sql,array('course_id'=>$course_id,
        											 'instance_id'=>$outcome_id));
    }
    
    /**
     * 
     * Returns the record from the course_module table that matches the given id 
     * @param int $coursemodule_id	the id of the record that will be retrieved from the course_modules table
     * 
     * @return mixed object record from the course_modules table that matches the given id or false 
     */
    
    function get_coursemodule($coursemodule_id)	{
    	return	$this->dbc->get_record('course_modules',array('id'=>$coursemodule_id));
    }
    
    /**
     * Returns the scale that a course uses
     *
     * @param int $course_id
     * @return object the scale as an object
     */
    function get_course_scale($course_id) {
        global $CFG;
    ...
    /**
     * Gets the assessor's final grade for the course.
     *
     * @param int $course_id the id of the course
     * @param int $candidate_id the id of the candidate
     * @return array containing the course grade
     */
    function get_course_grade($course_id, $candidate_id) {
        global $CFG;
     ...
     * @return void
     */
    function set_course_grade($course_id, $candidate_id, $scale_item_id, $comments) {
        global $CFG, $USER;
     ...
     * @return array of comments objects
     */
    function get_course_comments($course_id,$candidate_id) {

 ...
 */
class assmgr_moodleform extends moodleform {
    ...
    /**
     * This is identical to the overridden function except that it calls assmgr_MoodleQuickForm instead
     * of MoodleQuickForm
     * @param <type> $action
     ...
     * @param <type> $editable
     */
    function assmgr_moodleform($action=null, $customdata=null, $method='post', $target='', $attributes=null, $editable=true) {
        if (empty($action)){
        ...
        $this->_form =& new assmgr_MoodleQuickForm($this->_formname, $method, $action, $target, $attributes);
        if (!$editable){
 ...
 * and decoded properly. It is called when a new form is created by the assmgr_moodleform class
 */
class assmgr_MoodleQuickForm extends MoodleQuickForm {
     ...
     * @access   public
     */
    function assmgr_MoodleQuickForm($formName, $method, $action, $target='', $attributes=array()) {
    ...
    function setDefaults($defaultValues = null, $filter = null) {
        ...
        foreach ($this->_defaultValues as $name => &$value) {
            ...
            if ($this->getElementType($name) == 'htmleditor') {
                $this->_defaultValues[$name] = assmgr_db::decode($value);
            }
        }
